...
- 发布流程新增 `*-tiny.zip` 轻量安装包，移除默认字体资源，默认 logo 资源将替换为 1x1 占位图，不包含预压缩文件，且不注入 Mermaid / instant.page 运行时，用于进一步缩小安装包体积。
...
- 简体中文安装包文件名由 `howiehz-higan-cn*` 调整为 `howiehz-higan-zh-hans*`。
...
- 同步更新首页一言配置项的中英文文档与配置表单说明，统一改为 `quote` 命名。
...
- 将“总体样式 -> 额外菜单项”中“随机文章”实现由原先的纯模板实现替换为官方 Finder API 实现。
...
- 除文章详情页、独立页面和首页，其他页面均在原标题后附加 `- 站点标题`。

### 🐛 错误修复

- 修复作者页标题被意外固定为站点标题的问题
...
- 新增[“深色正文样式”](/guide/theme-configuration#深色正文样式)配置项（总体样式 -> 深色正文样式），提供 `轻柔灰` 选项，可让深色模式下的正文文字观感更柔和，减轻纯白文字带来的偏亮、偏粗观感。

### 🔧 代码重构

- 将[“行内代码样式”](/guide/theme-configuration#行内代码样式)从“文章页样式”调整为“总体样式”，统一作为全局样式配置项管理。

### 📄 文档

- 更新“行内代码样式”与“深色正文样式”文档，补充配置位置、适用范围与效果说明。
...
- 新增[“行内代码样式”](/guide/theme-configuration#行内代码样式)配置项（文章页样式 -> 行内代码样式），可为文章页和独立页面的行内代码选择样式，提供默认虚线边框、柔和高亮、主题色胶囊三种选项，不影响多行代码块。
...
- 主题的静态资源已提供预压缩版本：
  - 文件类型：
    - 当前实际构建产物主要包含 `.js`、`.css`、`.ttf`、`.ico` 的预压缩文件，例如 `.js` 对应 `.js.gz`、`.js.br`、`.js.zst`
  - 对应算法与压缩等级：
    - `*.gz` — gzip（压缩等级 9，最高）
